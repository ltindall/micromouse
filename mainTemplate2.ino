...
#define TURN_RIGHT_COUNT 2000
#define TURN_LEFT_COUNT 2000
#define TURN_AROUND_COUNT 4800

#define ONECELL 7500

//constants for wall sensing left/right/front
#define hasLeftWall 100//LUCAS FIX THIS
#define hasRightWall 140
#define hasFrontWall 450
...
//while(1){
//  readSensor();
//  if(check_right_wall() && !check_front_wall())
//    test_tone();
//  delay(500);
//}
...
void pid( void ) {
  ...
  if ((RFSensor > 550 && LFSensor > 550) || check_front_wall() ) //prevents mouse from crashing into wall due to errors.
...
//    Serial.println("only left wall"); 
    errorP = .15 * ( 200 - leftSensor );      // the error away from center with only a left wall
    errorD = errorP - oldErrorP;           // change in error
  }
  // sensor only reads in right wall
  else if(check_right_wall()){
//    Serial.println("only right wall"); 
    errorP = .15 * (rightSensor - 400 );      // the error away from center with only a right wall
    errorD = errorP - oldErrorP;           // change in error
  }
...
void motorbreak()
{
  drive.leftBackward(80);
  drive.rightBackward(80);
  delay(30);
  drive.leftBackward(0);
...
void center(){
  ...
  while( micros() - cur < 1000 ) {
    readSensor(); 
   if(leftSensor > rightSensor){
    drive.leftForward(leftSensor-rightSensor);
    drive.rightBackward(leftSensor-rightSensor);  
   }
   if(rightSensor > leftSensor){
    drive.rightForward(0.1*(rightSensor-leftSensor)); 
    drive.leftBackward(0.1*(rightSensor-leftSensor));  
   }
